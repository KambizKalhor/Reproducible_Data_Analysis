...
5.  Explain what the following code does. It may be helpful to reference the answer to the previous question:
    ...
    We can see vector recycling, when we perform some kind of operations like addition, subtraction, etc. on two vectors of unequal length. The vector with a small length will be repeated as long as the operation completes on the longer vector.
